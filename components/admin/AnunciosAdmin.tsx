...
export default function AnunciosAdmin() {
  ...
  const [novoAnuncio, setNovoAnuncio] =
    useState<FormNovoAnuncio>(novoAnuncioInicial);
  ...
  const [novaFotoEdicao, setNovaFotoEdicao] =
  ...
  const pendentes = anuncios.filter(
    (anuncio) => anuncio.status === 'pendente'
  ).length;

  const aprovados = anuncios.filter(
    (anuncio) =>
      anuncio.status === 'aprovado' || anuncio.aprovado === true
  ).length;

  const rejeitados = anuncios.filter(
    (anuncio) => anuncio.status === 'rejeitado'
  ).length;
   ...
   * IMPORTANTE:
   *
   * A policy atual do Storage exige que a primeira pasta
   * do arquivo seja o auth.uid() do usuário autenticado.
   *
   * Portanto:
   *
   * ID-DO-USUARIO/admin/arquivo.jpg
   *
   * ou:
   *
   * ID-DO-USUARIO/admin-edicao/ID-ANUNCIO/arquivo.jpg
   */
  const enviarImagem = async (
    ...
    if (userError) {
      console.error(
        'Erro ao identificar usuário para upload:',
        userError
      );
      throw userError;
    }
    ...
    if (!user) {
      throw new Error(
        'Usuário não autenticado. Entre novamente no painel administrativo.'
      );
    }
  ...
  const atualizarStatus = async (
    ...
    if (
      ...
      const continuar = confirm(
        'Este anúncio não possui foto. Deseja aprovar mesmo assim?'
      );
  ...
  const renovarAnuncio = async (anuncio: Anuncio) => {
    const dias =
      anuncio.plano_usado === 'Gratuito'
        ? 7
        : anuncio.plano_usado === 'Impulso'
          ? 15
          : 30;

    if (
      !confirm(
        `Deseja renovar este anúncio por mais ${dias} dias?`
      )
    ) {
      return;
    }
    ...
    const base =
      ...
      new Date(anuncio.data_expiracao).getTime() > Date.now()
  ...
  const excluirAnuncio = async (id: string) => {
    ...
    try {
      ...
      console.error(
        'Erro inesperado ao excluir anúncio:',
        error
      );
  ...
  const criarAnuncioManual = async () => {
    if (
      !novoAnuncio.titulo.trim() ||
      !novoAnuncio.nome_loja.trim() ||
      !novoAnuncio.categoria ||
      !novoAnuncio.cidade
    ) {
      alert(
        'Preencha título, anunciante, categoria e cidade.'
      );
      return;
    }

    const cidadeOficial = cidades.find(
      (cidade) => cidade.nome === novoAnuncio.cidade
    );
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert([
          {
            tipo: tipoPelaCategoria(
              novoAnuncio.categoria
            ),
            titulo: novoAnuncio.titulo.trim(),
            nome_loja: novoAnuncio.nome_loja.trim(),
            telefone:
              novoAnuncio.telefone.trim() || null,
            email: novoAnuncio.email.trim() || null,
            instagram:
              novoAnuncio.instagram.trim() || null,
            descricao:
              novoAnuncio.descricao.trim() || null,
            ...
            plano_usado:
              novoAnuncio.plano_usado,
            payment_status:
              novoAnuncio.payment_status,
            status: 'aprovado',
            aprovado: true,
            ativo: true,
            destaque:
  ...
  const abrirEdicaoAnuncio = (anuncio: Anuncio) => {
    setAnuncioEditando({
      ...
      plano_usado:
        anuncio.plano_usado || 'Gratuito',
      payment_status:
        anuncio.payment_status || 'pendente',
    ...
    setPrecoEdicao(
      formatarPrecoParaCampo(anuncio.preco)
    );
  ...
  const salvarEdicaoAnuncio = async () => {
    ...
    const cidadeOficial = cidades.find(
      (cidade) =>
        cidade.nome === anuncioEditando.cidade
    );
    ...
    try {
      ...
      if (novaFotoEdicao) {
        ...
        imagensAtualizadas = [
          novaUrl,
          ...imagensAtualizadas.filter(Boolean),
        ];
      }

      const status =
        anuncioEditando.status || 'pendente';
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          nome_loja:
            anuncioEditando.nome_loja?.trim() ||
            null,
          telefone:
            anuncioEditando.telefone?.trim() ||
            null,
          email:
            anuncioEditando.email?.trim() || null,
          instagram:
            anuncioEditando.instagram?.trim() ||
            null,
          descricao:
            anuncioEditando.descricao?.trim() ||
            null,
          preco: normalizarPreco(precoEdicao),
          categoria:
            anuncioEditando.categoria || null,
          tipo: tipoPelaCategoria(
            anuncioEditando.categoria || ''
          ),
          cidade: cidadeOficial.nome,
          estado: cidadeOficial.estado,
          plano_usado:
            anuncioEditando.plano_usado,
          payment_status:
            anuncioEditando.payment_status,
          status,
          aprovado,
          ativo: anuncioEditando.ativo ?? true,
          destaque:
            anuncioEditando.destaque ?? false,
          data_expiracao:
            anuncioEditando.data_expiracao ||
            null,
      ...
      console.error(
        'Erro ao salvar edição do anúncio:',
        error
      );
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Cadastre, aprove, edite e renove anúncios.
          </p>
        </div>
      ...
      <div className="bg-white border rounded-3xl p-5 mb-6">
        <div className="grid md:grid-cols-[1fr_200px_220px_auto] gap-4">
          ...
          <select
            value={filtroCidade}
            onChange={(event) =>
              setFiltroCidade(event.target.value)
            }
      ...
      {mostrarNovoAnuncio && (
        <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <SelecaoAdmin
              label="Pagamento"
              value={novoAnuncio.payment_status}
              onChange={(valor) =>
                setNovoAnuncio({
                  ...novoAnuncio,
          ...
          <div className="flex gap-4 mt-6">
            ...
            <button
              type="button"
              onClick={criarAnuncioManual}
              disabled={salvandoNovo}
              className="flex-1 py-4 bg-emerald-600 disabled:bg-slate-400 text-white rounded-2xl"
            >
              {salvandoNovo
                ? 'Salvando...'
                : 'Salvar anúncio'}
                  ...
                  >
                    <td className="p-4">
                      {anuncio.imagens?.[0] ? (
                        ...
                        <div className="w-20 h-20 bg-red-100 border border-red-200 rounded-xl flex items-center justify-center text-red-600 text-xs text-center">
                          Sem foto
                        </div>
                      )}
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex flex-wrap justify-center gap-2">
                        ...
                        <button
                          ...
                          className="bg-orange-600 text-white px-3 py-2 rounded-xl text-sm"
                        >
                          Renovar
                        </button>
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4">
            <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-4xl max-h-[92vh] overflow-y-auto">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={
                      anuncioEditando.data_expiracao
                        ? anuncioEditando.data_expiracao.slice(
                            0,
                            10
                          )
                        : ''
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-7">
                ...
                <button
                  type="button"
                  onClick={salvarEdicaoAnuncio}
                  disabled={salvandoEdicao}
                  className="flex-1 py-4 bg-green-600 disabled:bg-slate-400 text-white rounded-2xl"
                >
                  {salvandoEdicao
                    ? 'Salvando...'
                    : 'Salvar alterações'}
...
function Contador({
  ...
  return (
    <div
      className={`${classe} rounded-2xl p-5 text-center`}
    >
...
function Status({ status }: { status: string }) {
  ...
  return (
    <span
      className={`${classe} px-3 py-1 rounded-full text-xs font-bold`}
    >
      {status.toUpperCase()}
    </span>
  );
  ...
  return (
    <div>
      ...
      <input
        ...
        value={value}
        readOnly
        className="w-full border border-slate-200 rounded-2xl px-4 py-3 bg-slate-100 text-slate-600"
      />
    </div>
  );
}

function SelecaoAdmin({
  label,
  value,
  onChange,
  options,
